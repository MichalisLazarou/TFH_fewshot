...
class Attention(nn.Module):
    ...
    def forward(self, g_s, g_t):
        return torch.mean(torch.stack([self.at_loss(f_s, f_t) for f_s, f_t in zip(g_s, g_t)]))
    ...
    def at_loss(self, f_s, f_t):
        ...
        if s_H > t_H:
            ...
            pass
        #print((self.at(f_s) - self.at(f_t)).pow(2).mean())
        return (self.at(f_s) - self.at(f_t)).pow(2).mean()

    def at(self, f):
        #print(f.pow(self.p).mean(0).view(f.size(1), -1))
        return f.pow(self.p).mean(0).view(f.size(1), -1)
